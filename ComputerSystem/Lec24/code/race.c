...
/*
 * This code tests whether passing a shared pointer to pthread_create generates a race condition.
 * For CNT (default = 100) values of i, will execute:
 *     Pthread_create(&tid, NULL, thread, &i);
 * But, since there is a race condition between the different calls, can get multiple threads with the
 * same value of i, and therefore some values of i that don't show up at all.
